...
## 0. 19 JULY 2026 UPDATE — nav consolidation, new pages (READ BEFORE REBUILDING)

Pushed live 19 July 2026, EN + FR together. Summary of what changed on the deployed site:

- **Nav is now 6 items on EVERY page:** Accueil/Home · Portfolio · Modèles/Models · **Tarifs/Pricing**
  · **À propos/About** · **Contact**. Rate card, Simulator and Video brief left the nav; they are
  reached through the new Pricing hub page. On the three white form pages the nav marks
  **Tarifs/Pricing** as current.
- **Three NEW pages (hand-built static HTML, NOT generated by any toolchain):**
  - `pricing.html` / `pricing-en.html` — Pricing hub, dark design: 3 numbered journey cards
    (1 Grille tarifaire "Un coup d'œil" → 2 Simulateur "Essayez" → 3 Brief vidéo "Prêt à lancer")
    + "not sure where to start → contact" line.
  - `contact.html` / `contact-en.html` — dark design (deliberate exception to "forms are white":
    a contact page is not printable). Email/WhatsApp/Instagram cards + short Web3Forms message form
    (same public key). Intro deliberately minimal: no "I answer fast", no languages list (Lou's call).
  - `about.html` / `about-en.html` — bio moved off the homepage. Portrait slot currently holds a
    sketch placeholder ("IMAGE À VENIR / IMAGE TO COME"); Lou is supplying an AI-style portrait
    (character sheet with Lou Denim cap). **No Guadeloupe mention, no languages line — locked.**
- **Homepage (both languages) restructured:** hero → Portfolio/Models boxes → typewriter quote →
  **"Comment ça marche / How it works"** (44px title, subtitle "De votre produit à votre vidéo, en
  cinq étapes." / "From your product to your film, in five steps.", 5 steps in a 3+2 card grid,
  pink number dots, CTA buttons "Voir les tarifs/See the pricing" solid pink + "Me contacter/Get in
  touch" ghost) → footer. The 3 tool cards and the About section were REMOVED from the homepage.
  Pink CTA buttons are an approved exception to the pink-hover-only rule.
- **Models pages tagline:** now "Des modèles générés en IA, disponibles pour votre campagne.
  Choisissez un visage existant, ou décrivez le vôtre." / "AI generated models, available for your
  campaign. Pick an existing one or describe your own." (replaces the old "identité cohérente /
  consistent identity…staged" sentence, which violated §6).
- **Header alignment (Lou's explicit requirement: nothing moves between pages):** white pages'
  topnav geometry aligned to the dark pages (hbar max-width 1180→1220, gap 10→20, brand span
  13px/700/.14em→15px/600/.16em); simulator pages now load Jost weight 600 and use the same
  `'Jost',sans-serif` fallback stack as every other page. Verified by measuring bounding boxes:
  nav x=138, brand x=1102, langs x=1214, logo x=594 on all 14 pages.
- `add_topnav.py` was updated to generate the new 6-item nav with the corrected geometry, so a
  future regeneration of the white pages stays consistent.

**⚠️ GENERATOR DRIFT WARNING:** the dark-mode generators (`build_d20.py`, `build_models_d2.py`,
`build_travail_d2.py`) have NOT yet been updated for this redesign. Running them + `deploy_production.py`
today would resurrect the OLD nav (7 items), the homepage tool cards and inline About section, and the
old models tagline — silently reverting everything above. **Do not run the dark-mode rebuild until the
generators are updated to match the deployed 19 July pages.** The white-page toolchain is safe
(add_topnav.py is updated), but remember to re-run add_topnav.py after any white-page regeneration.
